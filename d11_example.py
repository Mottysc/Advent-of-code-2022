...
class Monkey():
    def __init__(self, id: int, holding: list, operation: str, test: int, testtrue: int, testfalse: int) -> None:
        self.id = id
...
active_monkeys = [
    Monkey(0, [74, 64, 74, 63, 53], "old * 7", 5, 1, 6),
    Monkey(1, [69, 99, 95, 62], "old * old", 17, 2, 5),
    Monkey(2, [59, 81], "old + 8", 7, 4, 3),
    Monkey(3, [50, 67, 63, 57, 63, 83, 97], "old + 4", 13, 0, 7),
    Monkey(4, [61, 94, 85, 52, 81, 90, 94, 70], "old + 3", 19, 7, 3),
    Monkey(5, [69], "old + 5", 3, 4, 2),
    Monkey(6, [54, 55, 58], "old + 7", 11, 1, 5),
    Monkey(7, [79, 51, 83, 88, 93, 76], "old * 3", 2, 0, 6)
]
global inactive_monkeys
global divisor
divisor = prod([monkey.test for monkey in active_monkeys])
inactive_monkeys = []
roundlog = []
for round_n in range(10000):
    for monkey in active_monkeys:
        #print(f"Monkey {monkey.id}:")
        for x in range(len(monkey.holding)):
            monkey.do_operation()
            try:
                active_monkeys[monkey.do_test()].receive_item(monkey.give_item())
            except IndexError:
                pass
    #print("\n".join([f"Monkey {monkey.id}: {', '.join([str(item) for item in monkey.holding])}" for monkey in active_monkeys]))
